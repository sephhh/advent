...
const buildRanges = (ruleInput: string[]) => {
    let ranges: RangeTuple[] = ruleInput.reduce((ranges: RangeTuple[], ruleLine: string) => {
...
const buildRangesMap = (ruleInput: string[]) => {
    return ruleInput.reduce((rangeRuleMap: RangeRuleMap, ruleLine: string) => {
...
const isInRange = (number: number, range: RangeTuple) => (number >= range[0] && number <= range[1]);
...
const solvePart1 = (input: string) => {
    ...
    for (let i = 1; i < ticketLines.length; i++) {
        const ticketValues = ticketLines[i].split(",").map((numString: string) => Number(numString));
...
const solvePart2 = (input: string) => {
    const [rules, yourTicket, nearbyTickets] = input.trim().split("\n\n");
    const rangeRuleMap: RangeRuleMap = buildRangesMap(rules.split("\n"));
    const nearbyTicketLines = nearbyTickets.split("\n");
    nearbyTicketLines.shift();
    const parsedNearbyTickets: number[][] = nearbyTicketLines.map(
        (ticketLine) => ticketLine.split(",").map((numString: string) => Number(numString))
    );
    let indexRuleMap = {};
    const fieldNames = Object.keys(rangeRuleMap);
    const validNearbyTickets = parsedNearbyTickets.filter((ticket) => {
        return ticket.every(
            (value) => fieldNames.find((fieldName) => {
...
const deduce = (indexRuleMap:{[key: string] : string[] }) => {
